    ...
    while (j < MAX_K)
    {
        ...
        for (int idx = 0; idx < cur_idxs.size(); ++idx)
        {
            if (cur_idxs[idx] < seq.size() && seq[cur_idxs[idx]] == mask[j])
                next_idxs.push_back(cur_idxs[idx]);
        }
        // cout << "nextids ";
        ...
        {
            return {{next_idxs[0] - j}, j + 1};
        }

        vector<int> new_cur_idxs;
        for (int loc : next_idxs)
        {
            if (loc + 1 < seq.size())
                new_cur_idxs.push_back(loc + 1);
        }
...
int main(int argc, char *argv[])
{
    ...
    // //read masks from file masks.txt space separated and each mask on newline
    // ifstream mask_file("masks.txt");
    // if (!mask_file.is_open())
    // {
    //     cerr << "Error: Could not open masks file masks.txt\n";
    //     return 1;
    // }
    // string line;
    ...
    ofstream sketch_file(out_dir + "/sketches.txt");
    if (!sketch_file.is_open())
    {
        cerr << "Error: Could not open output file " << out_dir + "/sketches.txt" << "\n";
        return 1;
    }
    for (int i = 0; i < sketches.size(); i++)
    {
        sketch_file << "Sketch " << i + 1 << ": ";
        for (auto x : sketches[i])
        {
            sketch_file << x << " ";
        }
        sketch_file << "\n";
    ...
    {
        out_file << pair.first.first+1 << "\t" << pair.first.second.first +1 << "\t" << pair.second << "\t" << pair.first.second.second<< "\n";
    }
    out_file.close();
